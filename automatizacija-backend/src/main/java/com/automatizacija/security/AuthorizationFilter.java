...
public class AuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
      if(request.getServletPath().contains("/api/login") || request.getServletPath().equals("/api/register") || 
    		  request.getServletPath().equals("/api/sokovi") || request.getServletPath().equals("/api/confirm") ||
    		  request.getServletPath().equals("/api/proizvodnje")){
          ...
          if(authorizationHeader != null && authorizationHeader.startsWith("Bearer")){
              try{
                  ...
                  String[] tipKorisnika = decodedJWT.getClaim("tipKorisnika").asArray(String.class);
